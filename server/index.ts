...
app.get('/companies', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  if (!req.userId) {
    return res.status(401).json({ error: 'Unauthorized: Missing user ID' });
  }
...
app.post('/company', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  if (!req.userId) {
    return res.status(401).json({ error: 'Unauthorized: Missing user ID' });
  }
  ...
  try {
    ...
    if (companyResult.rows.length > 0) {
      // Company already exists, return a different message
    ...
    // Insert new company with user_id = current user
...
app.delete('/companies/:companyId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  if (!req.userId) {
    return res.status(401).json({ error: 'Unauthorized: Missing user ID' });
  }
  ...
  try {
    // Only allow the user who added the company to delete it
    const result = await pool.query(
...
// Fetch all job boards endpoint.
app.get('/job-boards', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  if (!req.userId) {
    return res.status(401).json({ error: 'Unauthorized: Missing user ID' });
  }
...
app.post('/job-board', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  if (!req.userId) {
    return res.status(401).json({ error: 'Unauthorized: Missing user ID' });
  }
  ...
  if (!name) {
    return res.status(400).json({ error: 'Job board name is required.' });
  }

  try {
    // Check if a job board with that name already exists
    ...
    // Insert new job board with user_id = current user
    const insertResult = await pool.query(
...
app.delete('/job-boards/:jobBoardId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  if (!req.userId) {
    return res.status(401).json({ error: 'Unauthorized: Missing user ID' });
  }
  ...
  if (!jobBoardId) {
    return res.status(400).json({ error: 'Job board ID is required' });
  }

  try {
    // Only allow the user who added the job board to delete it
    const result = await pool.query(
