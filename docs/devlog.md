...
- Deliverables:<br>
    - <b>(Backend)</b>:
        - Endpoint for adding a company to the db using a company name passed in (POST /addcompany)
        - Endpoint for deleting a company from the db using a company name passed in (DELETE /removecompany)
        - Endpoint for fetching all companies from db (GET /companies)
        - Endpoint for editing a company entry from db (PATCH /companies/:companyId)
...
- Deliverables:<br>
    - <b>(Backend)</b>:
        - Endpoint for retrieving all tag/value combinations from the DB (GET /tagvalues)
        - Endpoint for adding new tag and value to the tagvalues table. The tag and value must be validated and cleaned before adding to DB (POST /tagvalue)
        - Endpoint for removing a given tag/value from the DB (DELETE /tagvalue)
        - Endpoint for patching a given tag/value from the DB (PATCH /tagvalue)
    - <b>(Frontend)</b>:
